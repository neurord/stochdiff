...
2. single compartment, cascade of reactions from metabotropic glutamate receptor through production of IP3
   Model_mglur.xml,Rxn_mglur.xml,IC_mglur.xml,Morph1comp.xml,  Out_mglur.xml 
...
3. Multiple voxels, cascade of reactions from metabotropic glutamate receptor through production of IP3
   Model_mglur_diff.xml,Rxn_mglur_diff.xml,IC_mglur.xml,Morph1comp.xml,  Out_mglur.xml 
   Uses defaultMaxElementSide of 3.0, to create multiple voxels
   use Rxn_mglur_diff.xml which has non-zero diffusion constants
...
4. Multiple voxels and regions by adding a spine to the morphology
   Model_mglur_spine-Ca(Y, N or Ntrain).xml, Rxn_Ca_mglur.xml, Out_Ca_mglur.xml
   ...
   Same reactions as previous: Glutamate through IP3
   Same output file specification
   stimulation in separate file: 
   ...
   b. Stim_mglur_Ca_spine.xml
		Injects glutamate and calcium.
		Since calcium synergistically activates Plc, this shows interaction between calcium and glutamate
   c. Stim_glu_train.xml 
		Injects two trains of glutamate
		increased glutamate increases the production of IP3
   
   ######## visualize morphology: Model_morph_spine.png, surface density initial condition: Model_IC_spine.png
   plot together using facility of Neurordh5_analysis.py that allows plotting of several models that use same morphology, etc.
   ######## neurordh5_analysis.py output shows the effect of calcium: Model_mglur_spine-Ca.png
...
5. 2D morphology with two types of spines
Shows how to create spine prototypes, and distribute them on a long dendrite
Model_2spinetypes.xml,Rxn_glubuf.xml, IC_glubuf.xml, Morph20um_2spinetypes.xml, Out_glu.xml
######## visualize morphology: Model_2spinetypes.png

visualize morphology of these :
######## Exception in thread "main" java.lang.NoClassDefFoundError: javax/vecmath/Tuple3d
6. 3D morphology with spines
Model_2spinetypes3D.xml,Rxn_glubuf.xml, IC_glubuf.xml, Morph20um_2spinetypes.xml, Out_glu.xml
Differs from above by 3D diffusion and curved voxels
...
8. branching morphology, Rxn_glubuf.xml, IC_glubuf.xml, Morph20um_2spinetypes.xml, Out_glu.xml
Shows how to create branched dendrites
######## Model_branchRad3.xml -> Model_branchRad3.png

To simulate these with Rxn_Ca_mglur.xml will need to create an IC file that either specifies only general concentration and surface density sets, or alternatively initialize molecules differently in the different branches.  Similarly, the stimulation will need to be specified somewhere other than into spines, either at ends of branches or into the submembrane of one of the segments 
